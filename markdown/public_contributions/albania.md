...
The `public contributions` by users in Albania on `2022/6/21 12:38 AM UTC`. This list contains users from `Albania` and cities `Tirana` `Durrës` `Vlorë` `Elbasan` `Shkodër` `Kamëz` `Fier` `Korçë`.

There are `9 countries` and `68 cities` can be found [here](https://github.com/MattyTheHacker/top-github-users).

There are `950 users`  in Albania. You need at least `0 followers` to be on this list.
